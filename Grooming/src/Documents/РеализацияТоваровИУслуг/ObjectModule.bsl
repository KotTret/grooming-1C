...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Пока ВыборкаНоменклатура.Следующий() Цикл
		Если ВыборкаНоменклатура.ЭтоТовар Тогда
			...
			Если Превышение > 0 И Константы.ЗапретПроведенияПриОтрицательныхОстатках.Получить() Тогда
				...
				Сообщение.Текст = СтрШаблон("Превышение остатка по номенклатуре ""%1""  в количестве ""%2""", ВыборкаНоменклатура.ТоварПредставление, Превышение);
				Сообщение.Сообщить();
				Отказ = Истина;
			...
			Если ЕстьОтрицательныеОстатки Тогда 
				...
				Движение.Сумма = 0;
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = СтрШаблон("По номенклатуре ""%1""  образовался отрицательный остаток в количестве ""%2""", ВыборкаНоменклатура.ТоварПредставление, ОтрицательныеОстатки);
				Сообщение.Сообщить();	
			КонецЕсли;
			...
			Если ЕстьОтрицательныеОстатки Тогда
				Движение.Сумма = СтоимостьОбщая + ВыборкаНоменклатура.СуммаВДокументе*ОтрицательныеОстатки / ВыборкаНоменклатура.КоличествоВДокументе;
			Иначе 
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.ЗаписьКлиента") И 
			Отказ = Ложь Тогда
