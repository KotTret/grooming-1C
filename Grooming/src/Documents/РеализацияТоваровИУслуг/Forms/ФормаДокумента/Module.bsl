...
Процедура ПриОткрытии(Отказ)
	Если Объект.Автор.Пустая() Тогда
		Объект.Автор  = УзнатьТекущегоПользователяКлиентПовтИсп.УзнатьТекущегоПользователя();
	КонецЕсли;

КонецПроцедуры
&НаСервере
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Если Не ЗначениеЗаполнено(Объект.Ссылка) И Документы.ЗаписьКлиента.ПроверитьОказаниеУслуг(Объект.Основание) Тогда
		Отказ = Истина;
